...
- The `deliveryPricing: {}` config slot is **opt-in** and used only for registering **custom** strategies. Built-in strategies do not need to be registered — they ship with core's type registry and are usable as soon as they're imported.
...
Built-in strategies are zero-config — import the factory, pass it inline to `methods.create`:
...
## Custom strategies: the `deliveryPricing: {}` slot

When an app needs a strategy beyond the built-ins, it registers a custom strategy by ID:
...
createCommerce({
  ...
  deliveryPricing: {
    'app:vip-tier': vipTierStrategy(),
    'app:remote-quote': remoteQuoteStrategy({ apiUrl: env.QUOTE_API }),
  },
})
```

Each registered strategy gets a matching factory generated by core's type machinery (or hand-written by the strategy author). Method creation then accepts it the same way:

```ts
await commerce.delivery.methods.create({
  adapter: 'leajlak',
  pricing: appVipTier({ tier: 'gold' }),
})
```

Strategy IDs use the `<scope>:<name>` convention (`app:`, `<plugin-id>:`) so they don't collide with the unprefixed built-ins.

When neither built-ins nor custom strategies cover a need, the slot is omitted entirely and the app falls back to built-ins only.
...
The discriminated union of all registered strategies (built-ins + `deliveryPricing` slot entries) becomes the type of `deliveryMethod.pricing` inputs. TS picks the right `TSettings` from the `strategy` tag.
...
A strategy package exports both the factory and the registration object. The registration goes into `deliveryPricing: {}`; the factory is the call-site API:
...
App config wires the strategy in:

```ts
createCommerce({
  deliveryPricing: { 'app:vip-tier': vipTierStrategy() },
})
...
The two are kept paired by their shared tag string; core's type machinery links the factory's discriminant to the registered strategy's calculator.
...
- Each registered strategy must implement `tag`, `validateSettings`, and `calculate`.
- Strategy tags must be unique within the union of built-ins + `deliveryPricing` entries.
...
The two registries (`delivery`, `deliveryPricing`) are deliberately independent. They compose at the method level.
...
This means platform operators choose which strategies are available to merchants by registering them in `deliveryPricing`. Adding a new strategy is an app-level change, not a per-merchant change.

## Packaging

| Package | Owns |
|---|---|
| `commerce-kit` (core) | `DeliveryPricingStrategy` interface, the `deliveryPricing` config slot, the `core:delivery-fee` calculation step, validation rules, built-in strategy tag union types |
| `@commerce-kit/delivery-pricing-strategies` | Built-in factories + calculators (`free`, `flat`, `distanceBased`, `tieredDistance`, `weight`, `zone`) + `DistanceProvider` interface + ready-made distance providers as sub-modules |
...
Built-in calculators register themselves with core's strategy registry on import — apps don't need to declare them in `deliveryPricing: {}`. The slot is reserved for custom strategies.
